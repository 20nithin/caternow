...
export default function VendorLogin() {
  const [step, setStep] = useState('phone'); // phone | otp | detecting | confirm_location
  ...
  const handleOtpSubmit = (e) => {
    ...
    if (!result.success) {
      setError(result.error);
      if (result.locked) setLockoutTimer(result.lockoutRemaining || 120);
      ...
      {step === 'otp' && (
        <form onSubmit={handleOtpSubmit} className="animate-fade-in">
          ...
          <div style={{
            ...
            <span>🛡️ {attemptsRemaining} attempts left</span>
            {(() => {
          ...
          <button type="submit" className="btn btn-primary btn-block btn-lg" disabled={lockoutTimer > 0}>
            Verify & Continue →
          </button>

          <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
            <button type="button" className="btn btn-secondary btn-block" onClick={handleChangeNumber}>
              ← Back
            </button>
            <button
              type="button"
